...
func WithdrawHandler(w http.ResponseWriter, r *http.Request) {
	...
	isLoggedIn, memberSeq, memberName := GetBoardSession(r)
	if !isLoggedIn {
		utils.JSONError(w, http.StatusUnauthorized, "로그인이 필요합니다")
		return
	}
	...
	if err != nil {
		...
		utils.JSONError(w, http.StatusInternalServerError, "회원탈퇴 처리 중 오류가 발생했습니다")
		return
	}
	...
	if customer.KakaoID != 0 {
		if err := UnlinkKakaoUser(customer.KakaoID); err != nil {
			...
			utils.JSONError(w, http.StatusInternalServerError, "카카오 연결 해제 중 오류가 발생했습니다. 다시 시도해주세요.")
			return
		}
	}
	...
	utils.JSONSuccessMessage(w, "회원탈퇴가 완료되었습니다")
}
