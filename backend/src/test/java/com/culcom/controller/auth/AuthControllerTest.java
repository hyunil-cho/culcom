...
class AuthControllerTest {
    ...
    class Login {
        ...
        void 로그인_성공() throws Exception {
            UserInfo user = UserInfo.builder()
                    .userId("admin").userPassword("pass").name("관리자").role(UserRole.ROOT).build();
            user = userInfoRepository.save(user);

            given(authService.authenticate("admin", "pass")).willReturn(Optional.of(user));
            given(authService.getManagedBranches(any())).willReturn(List.of(branch));
        ...
        void 아이디_비밀번호_불일치시_401() throws Exception {
            given(authService.authenticate(anyString(), anyString())).willReturn(Optional.empty());
    ...
    class SelectBranch {
        ...
        void ROOT가_아닌_사용자가_권한없는_지점_선택시_403() throws Exception {
            UserInfo staffUser = userInfoRepository.save(UserInfo.builder()
                    .userId("staff").userPassword("pass").name("스태프").role(UserRole.STAFF).build());

            CustomUserPrincipal staffPrincipal = new CustomUserPrincipal(
                    staffUser.getSeq(), "staff", "스태프", UserRole.STAFF, branch.getSeq());

            given(authService.getManagedBranches(any())).willReturn(List.of());
